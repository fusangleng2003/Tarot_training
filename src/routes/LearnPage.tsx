  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      {!revealed ? (
        ...
        <div className="flex-1 flex flex-col overflow-hidden px-6 py-3 animate-fade-in">
          {/* 顶部操作栏 */}
          <div className="flex items-center justify-between mb-3 shrink-0">
            ...
            <button
              onClick={handleDraw}
              className="px-4 py-1 rounded-full border border-mystic-gold/40 text-mystic-gold/80 text-sm hover:border-mystic-gold hover:text-mystic-gold transition-colors"
            >
              换一张
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-5 flex-1 min-h-0">
            ...
            <div className="flex flex-col gap-2 min-h-0">
              <div className="shrink-0 flex justify-center">
                {card.imageUrl ? (
                  <img
                    src={card.imageUrl}
                    alt={`${card.name} 正位`}
                    className="h-40 w-auto rounded-xl border-2 border-mystic-gold/40 shadow-lg"
                  />
                ) : (
                  <div className="h-40 w-28 rounded-xl border-2 border-mystic-gold/30 bg-gradient-to-b from-mystic-deep to-mystic-veil flex items-center justify-center">
              ...
              <div className="bg-mystic-deep/50 rounded-lg p-3 border border-mystic-veil flex-1 min-h-0 overflow-y-auto">
                <h3 className="text-sm font-heading text-mystic-gold mb-1.5">正位含义</h3>
              ...
              <div className="bg-mystic-deep/50 rounded-lg p-3 border border-mystic-veil flex-1 min-h-0 overflow-y-auto">
                <h3 className="text-sm font-heading text-mystic-gold mb-1.5">生活场景</h3>
            ...
            <div className="flex flex-col gap-2 min-h-0">
              <div className="shrink-0 flex justify-center">
                {card.imageUrl ? (
                  <img
                    src={card.imageUrl}
                    alt={`${card.name} 逆位`}
                    className="h-40 w-auto rounded-xl border-2 border-mystic-rose/40 shadow-lg rotate-180"
                  />
                ) : (
                  <div className="h-40 w-28 rounded-xl border-2 border-mystic-rose/30 bg-gradient-to-b from-mystic-deep to-mystic-veil flex items-center justify-center rotate-180">
                    <span className="text-mystic-rose text-3xl">✦</span>
                  </div>
                )}
              </div>
              <div className="shrink-0 text-center">
              ...
              <div className="bg-mystic-deep/50 rounded-lg p-3 border border-mystic-rose/20 flex-1 min-h-0 overflow-y-auto">
                <h3 className="text-sm font-heading text-mystic-rose mb-1.5">逆位含义</h3>
                <div className="flex flex-wrap gap-1 mb-2">
              ...
              <div className="bg-mystic-deep/50 rounded-lg p-3 border border-mystic-rose/20 flex-1 min-h-0 overflow-y-auto">
                <h3 className="text-sm font-heading text-mystic-rose mb-1.5">生活场景</h3>
                <ul className="space-y-1">
              ...
              <div className="bg-mystic-deep/50 rounded-lg p-3 border border-mystic-veil flex-1 min-h-0 flex flex-col">
                <h3 className="text-sm font-heading text-mystic-gold mb-1.5 shrink-0">我的笔记</h3>
                <textarea
                  value={note}
                  onChange={(e) => setNote(e.target.value)}
                  onBlur={() => saveNote(card.id, note)}
                  placeholder="写下你对这张牌的长期理解和联想..."
                  className="w-full flex-1 min-h-0 bg-mystic-void/50 border border-mystic-veil rounded p-2 text-mystic-moon text-sm resize-none focus:outline-none focus:border-mystic-glow placeholder:text-mystic-star/30"
                />
              </div>
            </div>
          </div>
